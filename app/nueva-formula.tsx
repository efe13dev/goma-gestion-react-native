...
export default function NuevaFormulaScreen() {
	...
	const mostrarFormularioAgregar = () => {
		console.log("Botón añadir ingrediente presionado");
	...
	const guardarFormula = async () => {
		...
		try {
			...
			if (resultado) {
				Alert.alert("Éxito", "La fórmula se ha guardado correctamente", [
					{
						text: "OK",
						onPress: () => {
				...
				Alert.alert(
					"Error",
					"Ya existe una fórmula con ese nombre o hubo un problema al guardar",
				);
			}
		} catch (error) {
			...
			Alert.alert(
				"Error",
				"Ocurrió un error al guardar la fórmula. Intente nuevamente.",
			);
		} finally {
			setIsLoading(false);
		}
