...
export const getFormulaById = async (id: string): Promise<Formula | null> => {
  try {
    ...
    const response = await fetch(`${API_URL}${FORMULAS_ENDPOINT}/${encodeURIComponent(name)}`);
...
export const addFormula = async (formula: Formula): Promise<boolean> => {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Error al agregar la fórmula:', errorData || response.status);
      return false;
    }
...
export const updateFormula = async (id: string, formula: Formula): Promise<boolean> => {
  try {
    const apiFormula = mapLocalToApi(formula);
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      console.error(`Error al actualizar la fórmula ${name}:`, errorData || response.status);
      return false;
    }
...
export const deleteFormula = async (id: string): Promise<boolean> => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      console.error(`Error al eliminar la fórmula ${name}:`, errorData || response.status);
      return false;
    }
...
export const addIngredient = async (formulaId: string, ingredient: Ingrediente): Promise<boolean> => {
  try {
    ...
    console.error(`Error al agregar ingrediente a la fórmula ${formulaId}:`, error);
    return false;
  }
};
...
export const updateIngredient = async (
  ...
  try {
    ...
    const formula = await getFormulaById(formulaId);
    if (!formula || ingredientIndex < 0 || ingredientIndex >= formula.ingredientes.length) {
      return false;
    }
    ...
    console.error(`Error al actualizar ingrediente en la fórmula ${formulaId}:`, error);
    return false;
  }
};
...
export const deleteIngredient = async (
  ...
  try {
    ...
    console.error(`Error al eliminar ingrediente de la fórmula ${formulaId}:`, error);
    return false;
  }
};
